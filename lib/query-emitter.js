...
QueryEmitter.prototype.queryPoll = function(callback) {
  ...
  this.db.queryPoll(this.collection, this.query, this.options, function(err, ids, extra) {
    ...
    if (idsDiff.length) {
      var removed = getRemoved(idsDiff, emitter.ids);
      emitter.ids = ids;      
      var inserted = getInserted(idsDiff);
      updateSetWithDiffs(emitter.idSet, inserted, removed);
      if (inserted.length) {
        var snapshotOptions = {};
...
QueryEmitter.prototype.queryPollDoc = function(id, callback) {
  ...
  this.db.queryPollDoc(this.collection, id, this.query, this.options, function(err, matches) {
    ...
    var inPreviousResultSet = emitter.idSet.has(id);

    if (!inPreviousResultSet && matches) {
      // Add doc to the collection. Order isn't important, so we'll just whack
      // it at the end
      emitter.idSet.add(id);
    ...
    if (inPreviousResultSet && !matches) {
      var previousIndex = emitter.ids.indexOf(id);
      if (previousIndex !== -1) {
        emitter.ids.splice(previousIndex, 1);
        emitter.idSet.delete(id);
        emitter.onDiff([new arraydiff.RemoveDiff(previousIndex, 1)]);
        return callback();
      }
    }
...
function getInserted(diff) {
  return getDiffItemsByType(diff, arraydiff.InsertDiff)
}

function getRemoved(diff, values) {
  const removeDiffs = getDiffItemsByType(diff, arraydiff.RemoveDiff)
  var removedValues = []
  for (var i = 0; i < removeDiffs.length; i++) {
    var { index, howMany } = removeDiffs[i]
    if (index > -1 && howMany > 0) {
      removedValues = removedValues.concat(values.slice(index, index + howMany))
    }
  }
  return removedValues
}

function getDiffItemsByType(diff, diffType) {
  var targetDiffs = [];
  for (var i = 0; i < diff.length; i++) {
    var item = diff[i];
    if (diffType === arraydiff.InsertDiff && item instanceof arraydiff.InsertDiff) {
      for (var j = 0; j < item.values.length; j++) {
        targetDiffs.push(item.values[j]);
      }
    } else if (diffType === arraydiff.RemoveDiff && item instanceof arraydiff.RemoveDiff) {
      targetDiffs.push(item)
    }
  }
  return targetDiffs;
}
...
function updateSetWithDiffs(set, inserted, removed) {
  for (var i = 0; i < inserted.length; i++) {
    set.add(inserted[i])
  }
  for (var j = 0; j < removed.length; j++) {
    set.delete(removed[j])
  }
}
