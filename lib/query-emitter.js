...
QueryEmitter.prototype.queryPoll = function(callback) {
  ...
  this.db.queryPoll(this.collection, this.query, this.options, function(err, ids, extra) {
    ...
    if (idsDiff.length) {
      var diffItems = getDiffItems(idsDiff, emitter.ids);
      var inserted = diffItems.inserted;
      var removed = diffItems.removed;
      emitter.ids = ids;      
...
function getDiffItems(diff, currentValues) {
  var removed = []
  var inserted = []
  for (var i = 0; i < diff.length; i++) {
    ...
    if (item instanceof arraydiff.InsertDiff) {
      ...
      if (item.index == null || item.howMany == null) {
        continue;
      }
      var values = currentValues.slice(item.index, item.index + item.howMany)
      for (var k = 0; k < values.length; k++) {
        removed.push(values[k]);
      }
    }
  }
